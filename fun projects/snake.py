...
# Should put the above in a function and call it in the main loop. and make it simple. and put a chick in it and make her lame and gay.


        #list(snake[0]) == list(segment):

  
    
# skull should appear at the head of the snake if it goes off the board
# snake should grow after eating rat
# snake should die if it eats itself    
...
#        [snake[0][0], snake[0][1] - 1, '🟢']]
#        snake.append(random.choice(directions))   
            
                #x = 0
        #while x in range(len(snake)): # Get to second+ item in snake list and update with each last position of snakehead. 
        #    if x != 0:
    ...
    #    if x >= 1:
    #        snake[x][0] = newSegment[x-1][0]
    #        snake[x][1] = newSegment[x-1][1]
    #    x = x + 1
    #    snake.append(newSegment) # Insert new segment at the end of the list!
            
#if len(snake) > 1:
#            last_position = list(snake[1])
#        else:
#            last_position = list(snake[0])